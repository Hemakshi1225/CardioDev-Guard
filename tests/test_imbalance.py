...
Threshold constants from analyzers.qa_analyzers:
  IMBALANCE_THRESHOLD          = 0.20  → minority fraction < 0.20 triggers finding
  IMBALANCE_CRITICAL_THRESHOLD = 0.05  → minority fraction < 0.05 triggers CRITICAL
...
# Deterministic datasets — no random, explicit values
DF_BALANCED   = pd.DataFrame({"TenYearCHD": [0] * 50 + [1] * 50})          # 50/50
DF_MILD       = pd.DataFrame({"TenYearCHD": [0] * 85 + [1] * 15})          # 85/15  → HIGH
DF_SEVERE     = pd.DataFrame({"TenYearCHD": [0] * 95 + [1] * 5})           # 95/5   → CRITICAL
...
# ── severe imbalance (95/5) → CRITICAL ───────────────────────────────────────
