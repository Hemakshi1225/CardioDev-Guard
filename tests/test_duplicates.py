...
# ── one duplicate pair ────────────────────────────────────────────────────────
# df_one_dup: rows 0 and 1 are identical → 1 duplicate (pandas counts
...
# ── all-duplicate dataset ─────────────────────────────────────────────────────
# df_all_dups: 3 identical rows → 2 duplicates, fraction = 2/3 ≈ 0.667 → CRITICAL
...
class TestAllDuplicates:
    ...
    def test_all_dups_severity_critical(self, tmp_path):
        # 2 duplicates out of 3 rows = 66.7% >= 50% threshold → CRITICAL
        df = pd.DataFrame({"x": [7, 7, 7], "y": [8, 8, 8]})
        path = _write_csv(df, str(tmp_path))
        result = analyze_duplicates(_make_request(path))
